...
public class RevPolishCalc {
  ...
  public final float evaluate(String what) throws InvalidExpressionException {
    float output = 0;
      ...
      if (scan.hasNext()) {
        do {
          if (scan.hasNextFloat()) {
            ...
            } else if (opp == Symbol.SMALLDIVIDE) {
              float opperand1 = values.pop();
              float opperand2 = values.pop();
              if (opperand2 == 0) {
                throw new InvalidExpressionException("Do not divide by zero");
              } else {
                this.values.push(opperand1 / opperand2);
              }
